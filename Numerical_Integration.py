...
def func6(x): return x**5 + 2*x**3 - x + 1
def func7(x): return math.sin(x**2)
def func8(x): return math.cos(x)**2
def func9(x): return math.exp(-x**2)
def func10(x): return math.exp(x) * math.sin(x)
def func11(x): return x * math.log(x) if x > 0 else float('inf')
def func12(x): return math.sin(x) / x if x != 0 else 1  # sinc(x), синус-кардинал
def func13(x): return math.sqrt(x) * math.log(x) if x > 0 else float('inf')
...
def main():
    ...
    if func in [func4, func5, func11, func13] and (a <= 0 or b <= 0):
